#include "lss_implicit_time_loop_3d.hpp"
...
boundary_3d_pair implicit_boundary::get_y(grid_config_2d_ptr const &grid_config_xz, matrix_3d const &next_solution)
{
...
boundary_3d_pair implicit_boundary::get_intermed_x(grid_config_2d_ptr const &grid_config_yz,
                                                   matrix_3d const &next_solution)
{
...
boundary_3d_pair implicit_boundary::get_intermed_z(grid_config_2d_ptr const &grid_config_xy,
                                                   matrix_3d const &next_solution)
{
...
void implicit_time_loop_3d::run(heat_splitting_method_3d_ptr const &solver_ptr,
                                ...
                                traverse_direction_enum const &traverse_dir, matrix_3d &prev_solution,
                                matrix_3d &next_solution)
{
    ...
    if (traverse_dir == traverse_direction_enum::Forward)
    {
        ...
        while (time_idx <= last_time_idx)
        {
            boundary_solver_ptr->solve(prev_solution, x_boundary_pair, y_upper_boundary_ptr, z_boundary_pair, time,
                                       next_solution);
            y_boundary_pair = implicit_boundary::get_y(grid_config->grid_13(), next_solution);
            x_inter_boundary_pair = implicit_boundary::get_intermed_x(grid_config->grid_23(), prev_solution);
            z_inter_boundary_pair = implicit_boundary::get_intermed_z(grid_config->grid_12(), prev_solution);
