/**
    ...
    @file      lss_heat_hundsdorfer_verwer_method.hpp
    @brief     Hundsdorfer-Verwer splitting
    @details   ~
    @author    Michal Sara
    @date      14.02.2022
...
class hundsdorfer_verwer_rhs
{
  ...
  public:
    ...
    static void rhs_intermed_3(heat_coefficients_2d_ptr const &cfs, grid_config_2d_ptr const &grid_cfg,
                               matrix_2d const &input, matrix_2d const &inhom_input, matrix_2d const &inhom_input_next,
                               double const &time, matrix_2d &solution);

    static void rhs_intermed_4(heat_coefficients_2d_ptr const &cfs, grid_config_2d_ptr const &grid_cfg,
                               std::size_t const &y_index, double const &y, matrix_2d const &input,
                               matrix_2d const &inhom_input, double const &time, container_t &solution);
...
class heat_hundsdorfer_verwer_method : public heat_splitting_method
{
  private:
    // constant:
  ...
  public:
    explicit heat_hundsdorfer_verwer_method(lss_tridiagonal_solver::tridiagonal_solver_ptr const &solvery_ptr,
