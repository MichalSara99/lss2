...
void craig_sneyd_rhs::rhs_intermed_1(heat_coefficients_2d_ptr const &cfs, grid_config_2d_ptr const &grid_cfg,
...
void craig_sneyd_rhs::rhs_intermed_1_source(heat_coefficients_2d_ptr const &cfs, grid_config_2d_ptr const &grid_cfg,
                                            std::size_t const &y_index, double const &y, matrix_2d const &input,
...
void craig_sneyd_rhs::rhs_intermed_2(heat_coefficients_2d_ptr const &cfs, grid_config_2d_ptr const &grid_cfg,
...
void craig_sneyd_rhs::rhs_intermed_3(heat_coefficients_2d_ptr const &cfs, grid_config_2d_ptr const &grid_cfg,
                                     std::size_t const &y_index, double const &y, matrix_2d const &input,
                                     matrix_2d const &inhom_input, matrix_2d const &inhom_input_next,
                                     double const &time, container_t &solution)
{
...
void craig_sneyd_rhs::rhs_final(heat_coefficients_2d_ptr const &cfs, grid_config_2d_ptr const &grid_cfg,
                                std::size_t const &x_index, double const &x, matrix_2d const &input,
                                matrix_2d const &inhom_input, double const &time, container_t &solution)
{
    auto const &M_tilde = cfs->M_tilde_;
...
void heat_craig_sneyd_method::solve(matrix_2d const &prev_solution, boundary_2d_pair const &horizontal_boundary_pair,
                                    ...
                                    matrix_2d &solution)
{
    ...
    for (std::size_t j = 1; j < coefficients_->space_size_y_ - 1; ++j)
    {
        ...
        craig_sneyd_rhs::rhs_intermed_1(coefficients_, grid_cfg_, j, y, prev_solution, time, rhs_);
    ...
    for (std::size_t i = 1; i < coefficients_->space_size_x_ - 1; ++i)
    {
        ...
        craig_sneyd_rhs::rhs_intermed_2(coefficients_, grid_cfg_, i, x, prev_solution, inter_solution_1, time, rhs_);
    ...
    for (std::size_t j = 1; j < coefficients_->space_size_y_ - 1; ++j)
    {
        y = grid_2d::value_2(grid_cfg_, j);
        split_0(y, time, low_, diag_, high_);
        craig_sneyd_rhs::rhs_intermed_3(coefficients_, grid_cfg_, j, y, prev_solution, inter_solution_1,
                                        inter_solution_2, time, rhs_);
        solvery_ptr_->set_diagonals(low_, diag_, high_);
        solvery_ptr_->set_rhs(rhs_);
        solvery_ptr_->solve(horizontal_boundary_pair, solution_v, time, y);
        inter_solution_3.column(j) = solution_v;
    }
    ...
    for (std::size_t i = 1; i < coefficients_->space_size_x_ - 1; ++i)
    {
        x = grid_2d::value_1(grid_cfg_, i);
        split_1(x, time, low_, diag_, high_);
        craig_sneyd_rhs::rhs_final(coefficients_, grid_cfg_, i, x, prev_solution, inter_solution_3, time, rhs_);
        solveru_ptr_->set_diagonals(low_, diag_, high_);
        solveru_ptr_->set_rhs(rhs_);
        solveru_ptr_->solve(vertical_boundary_pair, solution_v, time, x);
...
void heat_craig_sneyd_method::solve(matrix_2d const &prev_solution, boundary_2d_pair const &horizontal_boundary_pair,
                                    boundary_2d_pair const &vertical_boundary_pair, double const &time,
                                    std::function<double(double, double)> const &heat_source, matrix_2d &solution)
{
}
