#include "lss_heston_equation_t.hpp"
...
void impl_heston_equation_cuda_qr_solver_crank_nicolson()
{
    ...
    // exact heston:
    heston_exact heston(0.0, strike, rate, 0.0, sig_kappa, sig_theta, sig_sig, rho, maturity);
    double s{}, v{}, benchmark{};
    auto const grid_cfg = std::make_shared<grid_config_2d>(discretization_ptr);
    auto const grid_trans_cfg = std::make_shared<grid_transform_config_2d>(discretization_ptr, grid_config_hints_ptr);
    std::cout << "tp : FDM | Exact | Abs Diff\n";
    for (std::size_t i = 0; i < solution.rows(); ++i)
    {
        s = grid_2d::transformed_value_1(grid_trans_cfg, grid_2d::value_1(grid_cfg, i));
        for (std::size_t j = 0; j < solution.columns(); ++j)
        {
            v = grid_2d::transformed_value_2(grid_trans_cfg, grid_2d::value_2(grid_cfg, j));
            benchmark = heston.call(s, v);
            std::cout << "(" << i << "," << j << ") : " << solution(i, j) << " |  " << benchmark << " | "
                      << (solution(i, j) - benchmark) << '\n';
        }
    }
}
